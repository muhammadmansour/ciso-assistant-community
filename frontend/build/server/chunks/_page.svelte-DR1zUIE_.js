...
import { d as RequirementAssessmentSchema } from './schemas-vgtyOSI9.js';
import { p as page } from './index3-BpCge2eg.js';
import { A as AutocompleteSelect } from './crud-DA2NQw0x.js';
...
import { Fr as lockedassessment1, Fo as lockedrequirementassessmentmessage3, bs as description, XS as additionalinformation1, oL as typicalevidence1, XP as adminnoteshelp2, XQ as adminnotes1, qZ as suggestedreferencecontrols2, pH as threatscovered1, EB as mappinginference1, EA as mappinginferencehelptext3, PY as coveragecolon1, sZ as scoresemicolon2, qX as suggestioncolon1, cF as status, ee as result, vA as requirementappliedcontrolhelptext4, kq as suggestcontrols1, gp as addappliedcontrol2, WR as appliedcontrolsplaceholder2, dy as appliedcontrols1, vv as requirementassessmentstatushelptext4, vw as requirementassessmentresulthelptext4, JL as extendedresulthelptext3, ef as extendedresult1, Ma as documentationscore1, sV as scoringhelptext2 } from './_index-BQcvYRD4.js';
...
import { b as complianceResultColorMap } from './constants-12fjCMiL.js';
...
function _page($$payload, $$props) {
  ...
  function extractAdminNoteText(item) {
  ...
  const adminNotesLines = Array.isArray(raw_admin_notes) ? raw_admin_notes.map(extractAdminNoteText).filter((v) => v !== null) : raw_admin_notes && typeof raw_admin_notes === "object" ? Object.values(raw_admin_notes).map(extractAdminNoteText).filter((v) => v !== null) : typeof raw_admin_notes === "string" && raw_admin_notes.trim().length > 0 ? raw_admin_notes.split("\n").filter((n) => n.trim().length > 0) : [];
  ...
  $$payload.out += `<!--]--> <div class="flex items-start justify-between mb-6 bg-white rounded-xl border border-gray-100 shadow-sm px-6 py-5"><div class="flex items-start gap-3"><a class="mt-1.5 text-gray-400 hover:text-[#005FA3] transition-colors"${attr("href", complianceAssessmentURL)} aria-label="Go back"><i class="fa-solid fa-arrow-left text-lg"></i></a> <div><div class="flex items-center gap-3 mb-1"><h1 class="text-2xl font-bold text-gray-900">${escape_html(data.requirement.ref_id)}</h1></div> <p class="text-sm text-gray-500">${escape_html(data.requirementAssessment.name)}</p></div></div> <button type="button" class="btn bg-[#005FA3] text-white hover:bg-[#004d85] transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 rounded-lg px-5 py-2.5"${attr("disabled", data.requirementAssessment.compliance_assessment.is_locked, true)} title="Run AI Analysis on Associated Evidences">`;
  {
    $$payload.out += "<!--[!-->";
    ...
    let children = function($$payload2, { form: form2, data: data2 }) {
      ...
      if (has_threats || has_reference_controls || annotation || mappingInference.result || typical_evidence || has_admin_notes) {
        ...
        if (!hideSuggestion) {
          ...
          if (typical_evidence) {
            $$payload2.out += "<!--[-->";
            const each_array_1 = ensure_array_like(typicalEvidenceLines);
            ...
            for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
              ...
              if (line.trim().includes("[EXCLUDED]")) {
                $$payload2.out += "<!--[-->";
                $$payload2.out += `<div class="opacity-50 flex items-start gap-1"><span class="inline-flex items-center shrink-0 mt-0.5 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-100 text-amber-700 border border-amber-200"><i class="fa-solid fa-ban mr-1 text-[8px]"></i>Excluded from AI Analysis</span> `;
                ...
                $$payload2.out += "<!--[1-->";
                MarkdownRenderer($$payload2, { content: line });
              } else {
                $$payload2.out += "<!--[!-->";
              }
              $$payload2.out += `<!--]-->`;
          ...
          if (has_admin_notes) {
            $$payload2.out += "<!--[-->";
            const each_array_2 = ensure_array_like(adminNotesLines);
            $$payload2.out += `<div class="mb-3"><p class="font-semibold text-xs uppercase tracking-wider text-[#1D53DA] mb-1.5"${attr("title", adminnoteshelp2())}><i class="fa-solid fa-user-shield mr-1.5 text-[#1D53DA]"></i> ${escape_html(adminnotes1())}</p> <div class="text-sm text-gray-600 leading-relaxed" dir="auto"><!--[-->`;
          ...
          if (has_threats || has_reference_controls) {
            ...
            if (reference_controls.length > 0) {
              $$payload2.out += "<!--[-->";
              const each_array_3 = ensure_array_like(reference_controls);
            ...
            if (threats.length > 0) {
              $$payload2.out += "<!--[-->";
              const each_array_4 = ensure_array_like(threats);
              ...
              for (let $$index_4 = 0, $$length = each_array_4.length; $$index_4 < $$length; $$index_4++) {
                let threat = each_array_4[$$index_4];
                $$payload2.out += `<li>`;
                if (threat.id) {
                  $$payload2.out += "<!--[-->";
      ...
      if (page.data.requirementAssessment.requirement.questions != null && Object.keys(page.data.requirementAssessment.requirement.questions).length !== 0) {
        ...
        {
          $$payload2.out += "<!--[-->";
          const each_array_5 = ensure_array_like(questionEntries);
          $$payload2.out += `<div class="px-5 pb-5 space-y-3"><!--[-->`;
          for (let idx = 0, $$length = each_array_5.length; idx < $$length; idx++) {
            let [urn, question] = each_array_5[idx];
            const currentAnswer = data2?.answers?.[urn];
            if (isQuestionVisible(question, data2?.answers || {})) {
              $$payload2.out += "<!--[-->";
              $$payload2.out += `<div${attr_class(`border border-gray-200 rounded-xl px-5 py-4 ${stringify(question.excluded ? "opacity-50" : "")}`)}><div class="flex items-start gap-4 mb-3"><div class="shrink-0 min-w-[60px]"><div class="text-sm font-bold text-gray-500">Q${escape_html(idx + 1)}</div> <div class="text-[11px] text-gray-400 capitalize whitespace-nowrap">${escape_html(question.type === "unique_choice" ? "Unique Choice" : question.type === "multiple_choice" ? "Multiple Choice" : question.type?.replace(/_/g, " ") || "")}</div></div> <div class="flex-1 text-sm text-gray-700 font-medium" dir="auto">${escape_html(question.text)} `;
              if (question.excluded) {
                $$payload2.out += "<!--[-->";
                $$payload2.out += `<span class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-100 text-amber-700 border border-amber-200"><i class="fa-solid fa-ban mr-1 text-[8px]"></i>Excluded from AI Analysis</span>`;
              } else {
                $$payload2.out += "<!--[!-->";
              }
              $$payload2.out += `<!--]--></div></div> `;
              if (question.type === "unique_choice") {
                $$payload2.out += "<!--[-->";
                const each_array_6 = ensure_array_like(question.choices);
                ...
                $$payload2.out += "<!--[1-->";
                const each_array_7 = ensure_array_like(question.choices);
                $$payload2.out += `<div class="flex flex-row flex-wrap gap-1.5"><!--[-->`;
...
//# sourceMappingURL=_page.svelte-DR1zUIE_.js.map
