...
import { f as complianceStatusColorMap, b as complianceResultColorMap } from './constants-BZXIbVIt.js';
import { e as getRequirementTitle, f as formatScoreValue, d as displayScoreColor } from './helpers-Bm9n0CNG.js';
import { s as safeTranslate, t as toCamelCase } from './i18n-WNCV45cf.js';
import { h as hideSuggestions } from './stores-CMqbeBUT.js';
import { b3 as description, VC as additionalinformation1, qo as suggestedreferencecontrols2, p8 as threatscovered1, jS as annotation, oh as typicalevidence1, Dc as mappinginference1, Db as mappinginferencehelptext3, Oh as coveragecolon1, qm as suggestioncolon1, vJ as questions, cr as observation, S9 as back, uM as requirementappliedcontrolhelptext4, uD as requirementevidencehelptext3, d7 as appliedcontrols1, cB as evidences } from './_index-DEXNURl5.js';
...
function _page($$payload, $$props) {
  ...
  if (data.requirementAssessment.requirement.questions != null && Object.keys(data.requirementAssessment.requirement.questions).length !== 0) {
    $$payload.out += "<!--[-->";
    const each_array_3 = ensure_array_like(Object.entries(data.requirementAssessment.requirement.questions));
    $$payload.out += `<h1 class="font-semibold text-sm">${escape_html(questions())}</h1> <!--[-->`;
    for (let $$index_3 = 0, $$length = each_array_3.length; $$index_3 < $$length; $$index_3++) {
      let [urn, question] = each_array_3[$$index_3];
      $$payload.out += `<li class="flex justify-between items-center border rounded-xl p-2 disabled"><p>${escape_html(question.text)} (${escape_html(safeTranslate(question.type))})</p></li>`;
    }
    $$payload.out += `<!--]-->`;
  } else {
    $$payload.out += "<!--[!-->";
  ...
  $$payload.out += `<!--]--> <div class="flex flex-row justify-between space-x-4"><button class="btn bg-gray-400 text-white font-semibold w-full" type="button">${escape_html(back())}</button></div></div>`;
  if ($$store_subs) unsubscribe_stores($$store_subs);
  pop();
}

export { _page as default };
//# sourceMappingURL=_page.svelte-BIXTnId_.js.map
