...
import { d as RequirementAssessmentSchema } from './schemas-Cmsh2Wi5.js';
import { p as page } from './index3-BwfRm5YV.js';
import { A as AutocompleteSelect } from './crud-BiYAuEEm.js';
...
import { ED as lockedassessment1, EA as lockedrequirementassessmentmessage3, bo as description, Wi as additionalinformation1, oF as typicalevidence1, qK as suggestedreferencecontrols2, pv as threatscovered1, DN as mappinginference1, DM as mappinginferencehelptext3, OU as coveragecolon1, sA as scoresemicolon2, qI as suggestioncolon1, cA as status, e9 as result, vb as requirementappliedcontrolhelptext4, kn as suggestcontrols1, gm as addappliedcontrol2, VB as appliedcontrolsplaceholder2, v2 as requirementevidencehelptext3, XX as addevidence1, WN as addsecurityexception2, dt as appliedcontrols1, cW as evidences, c4 as securityexceptions1, v6 as requirementassessmentstatushelptext4, v7 as requirementassessmentresulthelptext4, IF as extendedresulthelptext3, ea as extendedresult1, L3 as documentationscore1, sw as scoringhelptext2 } from './_index-BNamVw9A.js';
...
function _page($$payload, $$props) {
  ...
  let latestAnalysisQuestions = (() => {
    if (!localAiAnalyses || localAiAnalyses.length === 0) return [];
    const latest = localAiAnalyses.find((a) => a.status === "completed") || localAiAnalyses[0];
    if (!latest) return [];
    const qa = latest.question_answers || {};
    const result2 = latest.result || {};
    let questionSection = [];
    ...
    return entries.map((entry, idx) => {
      ...
      const confidenceValue = rawConf !== void 0 && rawConf !== null ? Number(rawConf) <= 1 ? Math.round(Number(rawConf) * 100) : Math.round(Number(rawConf)) : null;
      const qUrn = questionUrns[idx];
      const qDef = qUrn ? reqQuestions[qUrn] : null;
        ...
        $$payload2.out += `<div class="card bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden relative"><button type="button" class="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition-colors"><span class="flex items-center gap-2.5"><i class="fa-solid fa-robot text-[#005FA3]"></i> <span class="text-sm font-semibold text-gray-800">Assessment Questions</span> <span class="inline-flex items-center justify-center min-w-[22px] h-[22px] px-1.5 rounded-full text-xs font-semibold bg-[#005FA3]/10 text-[#005FA3]">${escape_html(questionEntries.length)}</span></span> <i${attr_class(`fa-solid ${stringify("fa-chevron-up")} text-gray-400 text-xs`)}></i></button> `;
        {
          ...
          for (let idx = 0, $$length = each_array_3.length; idx < $$length; idx++) {
            ...
            if (isQuestionVisible(question, data2?.answers || {})) {
              ...
              if (aiQ) {
                $$payload2.out += "<!--[-->";
                $$payload2.out += `<div class="flex items-center gap-3 shrink-0"><span${attr_class(`text-xs px-2 py-0.5 rounded-full font-semibold ${stringify(aiQ.answer === "Yes" ? "bg-green-100 text-green-700" : aiQ.answer === "No" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700")}`)}>AI: ${escape_html(aiQ.answer)}</span> `;
                if (aiQ.confidence !== null && aiQ.confidence !== void 0) {
                  $$payload2.out += "<!--[-->";
                  $$payload2.out += `<span${attr_class(`text-xs font-semibold ${stringify(aiQ.confidence >= 80 ? "text-green-600" : aiQ.confidence >= 50 ? "text-amber-600" : "text-red-600")}`)}>${escape_html(aiQ.confidence)}%</span>`;
                } else {
                  $$payload2.out += "<!--[!-->";
                }
                $$payload2.out += `<!--]--></div>`;
...
//# sourceMappingURL=_page.svelte-UnJbFXee.js.map
