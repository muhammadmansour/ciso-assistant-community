import { p as push, M as store_get, O as copy_payload, P as assign_payload, Q as unsubscribe_stores, a as pop, W as ensure_array_like, V as escape_html, T as attr, S as attr_class, X as stringify, Z as attr_style, R as bind_props } from './index2-9icAqEyj.js';
...
import { e as extendedResultColorMap, b as complianceResultColorMap, f as complianceStatusColorMap } from './constants-BZXIbVIt.js';
...
import { E2 as lockedassessment1, E1 as lockedassessmentmessage2, O6 as createdat1, e9 as maturity, bF as compliance, dR as extendedresult1, cA as progress, SK as associatedrequirements1, Da as mappinginferencetip2, e as edit, Y5 as actionplan1, cB as evidences, z1 as powerups1, Hm as flashmode1, aT as tablemode1, UU as applymapping1, Pq as cloneaudit1, Pe as comparetoaudit2, uQ as requestvalidation1, pR as synctoappliedcontrols3, k2 as suggestcontrols1, z2 as potentialthreats1, vO as questionplural1, vN as questionsingular1, AU as objectsnotvisible2, dQ as result, cf as status, Yt as showonlyassessable3, ku as yes, Ca as no, HE as filters, c8 as complianceassessment1, UK as ascsv3, UG as asxlsx4, UH as asword1, UF as aszip3, UI as aspdf3, Ih as exportbutton1 } from './_index-DEXNURl5.js';
import { f as auditFiltersStore, g as expandedNodesState } from './stores-CMqbeBUT.js';
...
function _page($$payload, $$props) {
  ...
  const objectsNotVisibleLabel = (count) => {
    return objectsnotvisible2({ count });
  };
  ...
  function $$render_inner($$payload2) {
    const each_array = ensure_array_like(Object.entries(data.compliance_assessment).filter(([key, value]) => {
      const fieldsToShow = [
        "ref_id",
        "name",
        "description",
    ...
    $$payload2.out += `<!--]--> <h1 class="text-xl font-semibold text-[#0A1628]">${escape_html(data.compliance_assessment.name)}</h1> <div class="flex gap-6"><div class="flex-1 space-y-6 min-w-0"><div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm"><div class="grid grid-cols-2 gap-x-8 gap-y-4"><!--[-->`;
    for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
      let [key, value] = each_array[$$index_1];
      $$payload2.out += `<div><h3 class="text-sm font-medium text-gray-500 mb-1 capitalize-first"${attr("data-testid", key.replaceAll("_", "-") + "-field-title")}>${escape_html(safeTranslate(key))}:</h3> <div class="text-sm text-gray-900"${attr("data-testid", key.replaceAll("_", "-") + "-field-value")}>`;
      if (value) {
        $$payload2.out += "<!--[-->";
        if (Array.isArray(value)) {
          ...
          if (visibleValues.length > 0) {
            $$payload2.out += "<!--[-->";
            const each_array_1 = ensure_array_like(visibleValues);
            $$payload2.out += `<!--[-->`;
              ...
              if (val.str && val.id) {
                ...
                if (!page.data.user.is_third_party) {
                  ...
                  Anchor($$payload2, {
                    ...
                    children: ($$payload3) => {
                      $$payload3.out += `<!---->${escape_html(val.str)}`;
                    },
                    $$slots: { default: true }
                  });
                } else {
                  $$payload2.out += "<!--[!-->";
                  $$payload2.out += `<span>${escape_html(val.str)}</span>`;
                }
                $$payload2.out += `<!--]-->`;
              } else if (val.str) {
                $$payload2.out += "<!--[1-->";
                ...
                $$payload2.out += "<!--[!-->";
                $$payload2.out += `<span>${escape_html(safeTranslate(val))}</span>`;
              }
              $$payload2.out += `<!--]--> `;
            ...
            if (hiddenCount > 0) {
              $$payload2.out += "<!--[-->";
              $$payload2.out += `<p class="mt-1 text-xs text-yellow-700">${escape_html(objectsNotVisibleLabel(hiddenCount))}</p>`;
            } else {
              $$payload2.out += "<!--[!-->";
            }
            $$payload2.out += `<!--]-->`;
            ...
            $$payload2.out += "<!--[1-->";
            $$payload2.out += `<p class="text-xs text-yellow-700">${escape_html(objectsNotVisibleLabel(hiddenCount))}</p>`;
          } else {
            $$payload2.out += "<!--[!-->";
          ...
          $$payload2.out += "<!--[1-->";
          const itemHref = `/${URL_MODEL_MAP["compliance-assessments"]["foreignKeyFields"]?.find((item) => item.field === key)?.urlModel}/${value.id}`;
          if (!page.data.user.is_third_party) {
            $$payload2.out += "<!--[-->";
            Anchor($$payload2, {
              href: itemHref,
            ...
            $$payload2.out += "<!--[!-->";
            $$payload2.out += `<span>${escape_html(value.str)}</span>`;
          }
          $$payload2.out += `<!--]-->`;
        } else if (isMaskedPlaceholder(value)) {
          $$payload2.out += "<!--[2-->";
          $$payload2.out += `<p class="text-xs text-yellow-700">${escape_html(objectsNotVisibleLabel(1))}</p>`;
        } else if (key === "description") {
          $$payload2.out += "<!--[3-->";
          MarkdownRenderer($$payload2, { content: value });
        } else if (key === "status") {
          $$payload2.out += "<!--[4-->";
          const statusLabel = safeTranslate(value.str ?? value);
          $$payload2.out += `<span${attr_class(`inline-block px-2.5 py-1 rounded-md text-xs font-medium ${stringify(value === "active" || value.str === "Active" ? "bg-green-100 text-green-800" : value === "in_progress" || value.str === "In Progress" ? "bg-blue-100 text-blue-800" : value === "done" || value.str === "Completed" || value.str === "Done" ? "bg-sky-100 text-sky-800" : "bg-gray-100 text-gray-800")}`)}>${escape_html(statusLabel)}</span>`;
        } else {
          $$payload2.out += "<!--[!-->";
          $$payload2.out += `${escape_html(safeTranslate(value.str ?? value))}`;
        }
        $$payload2.out += `<!--]-->`;
      } else {
        $$payload2.out += "<!--[!-->";
    ...
    $$payload2.out += `<!--]--> <div><h3 class="text-sm font-medium text-gray-500 mb-1">${escape_html(createdat1())}:</h3> <p class="text-sm text-gray-900">${escape_html(formatDateOrDateTime(data.compliance_assessment.created_at, getLocale()))}</p></div></div> `;
    if (page.data?.featureflags?.validation_flows) {
      $$payload2.out += "<!--[-->";
      $$payload2.out += `<div class="mt-4 pt-4 border-t border-gray-100"><!---->`;
      {
    ...
    $$payload2.out += `<!--]--></div> <!---->`;
    {
      $$payload2.out += `<div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm"><div class="flex items-start gap-6">`;
    ...
    $$payload2.out += `<!----> <div class="bg-white rounded-xl border border-gray-200 shadow-sm"><div class="p-6 border-b border-gray-200 flex items-center justify-between"><div class="flex items-center gap-3"><h2 class="text-lg font-semibold text-gray-900">${escape_html(associatedrequirements1())}</h2> <span class="bg-gray-100 text-gray-600 px-2.5 py-1 rounded-md text-sm font-medium">`;
    if (treeViewNodes) {
      $$payload2.out += "<!--[-->";
    ...
    $$payload2.out += `<!----></div> <div class="px-6 py-2"><div class="flex items-center text-xs text-gray-400 gap-2 py-2"><i class="fa-solid fa-diagram-project"></i> <p>${escape_html(mappinginferencetip2())}</p></div> <!---->`;
    {
      $$payload2.out += `<!---->`;
      {
    ...
    $$payload2.out += `<!----></div></div></div> <div class="w-72 flex-shrink-0 space-y-3">`;
    if (canEditObject) {
      $$payload2.out += "<!--[-->";
      Anchor($$payload2, {
        breadcrumbAction: "push",
    ...
    if (!page.data.user.is_third_party) {
      ...
      Anchor($$payload2, {
        ...
        children: ($$payload3) => {
          $$payload3.out += `<i class="fa-solid fa-heart-pulse w-4 h-4"></i> <span>${escape_html(actionplan1())}</span>`;
        },
        $$slots: { default: true }
      });
      $$payload2.out += `<!----> `;
      Anchor($$payload2, {
        ...
        class: "unstyled w-full px-4 py-3 bg-white text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-2 text-sm font-medium",
        children: ($$payload3) => {
          $$payload3.out += `<i class="fa-solid fa-file-lines w-4 h-4"></i> <span>${escape_html(evidences())}</span>`;
        },
        $$slots: { default: true }
      });
      $$payload2.out += `<!---->`;
    ...
    if (!page.data.user.is_third_party) {
      $$payload2.out += "<!--[-->";
      $$payload2.out += `<button class="w-full px-4 py-2.5 bg-white text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm flex items-center gap-2" data-testid="apply-mapping-button"><i class="fa-solid fa-diagram-project w-4"></i> ${escape_html(applymapping1())}</button> <button class="w-full px-4 py-2.5 bg-white text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm flex items-center gap-2" data-testid="clone-audit-button"><i class="fa-solid fa-copy w-4"></i> ${escape_html(cloneaudit1())}</button> <button class="w-full px-4 py-2.5 bg-white text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm flex items-center gap-2" data-testid="compare-audit-button"><i class="fa-solid fa-code-compare w-4"></i> ${escape_html(comparetoaudit2())}</button> `;
      if (page.data?.featureflags?.validation_flows) {
        $$payload2.out += "<!--[-->";
        $$payload2.out += `<button class="w-full px-4 py-2.5 bg-white text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm flex items-center gap-2" data-testid="request-validation-button"><i class="fa-solid fa-check-circle w-4"></i> ${escape_html(requestvalidation1())}</button>`;
      } else {
        $$payload2.out += "<!--[!-->";
      }
      $$payload2.out += `<!--]-->`;
    ...
    if (!page.data.user.is_third_party && !data.compliance_assessment.is_locked) {
      $$payload2.out += "<!--[-->";
      $$payload2.out += `<button class="w-full px-4 py-2.5 bg-white text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm flex items-center gap-2" data-testid="sync-to-actions-button">`;
      {
        $$payload2.out += "<!--[!-->";
        $$payload2.out += `<i class="fa-solid fa-arrows-rotate w-4"></i>`;
      }
      $$payload2.out += `<!--]--> ${escape_html(synctoappliedcontrols3())}</button>`;
    } else {
      $$payload2.out += "<!--[!-->";
    }
    $$payload2.out += `<!--]--> `;
    ...
    $$payload2.out += `<!--]--> <!---->`;
    {
      if (data.global_score.score >= 0) {
        $$payload2.out += "<!--[-->";
        $$payload2.out += `<div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm"><h3 class="text-sm font-semibold text-gray-900 mb-4">Progress</h3> <div class="flex items-center justify-center"><div class="relative">`;
        ProgressRing($$payload2, {
          strokeWidth: "14px",
          meterStroke: displayScoreColor(data.global_score.score, data.global_score.max_score),
          value: data.global_score.score * 100 / data.global_score.max_score,
          size: "size-32",
          children: ($$payload3) => {
            $$payload3.out += `<div class="flex flex-col items-center"><span class="text-xl font-bold text-gray-900">${escape_html(Math.round(data.global_score.score * 100 / data.global_score.max_score))}%</span> <span class="text-xs text-gray-500">completed</span></div>`;
          },
          $$slots: { default: true }
        });
        $$payload2.out += `<!----></div></div></div>`;
...
//# sourceMappingURL=_page.svelte-BLlYXxuW.js.map
