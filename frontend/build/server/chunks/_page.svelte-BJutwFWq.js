...
import { e as extendedResultColorMap, b as complianceResultColorMap, f as complianceStatusColorMap } from './constants-CbUNxZZz.js';
...
import { Fe as lockedassessment1, Fd as lockedassessmentmessage2, dO as framework, cC as perimeter, PA as createdat1, dx as appliedcontrols1, cE as status, gj as maturity, c2 as compliance, ee as extendedresult1, cZ as progress, Ur as associatedrequirements1, QV as collapseall1, JQ as expandall1, Em as mappinginferencetip2, vD as requestvalidation1, e as edit, c_ as evidences, _a as actionplan1, LR as domainscoverage1, zV as powerups1, IF as flashmode1, bg as tablemode1, Q_ as cloneaudit1, QM as comparetoaudit2, kp as suggestcontrols1, zW as potentialthreats1, wE as questionplural1, wD as questionsingular1, ed as result, _y as showonlyassessable3, kR as yes, Dh as no, IY as filters, cx as complianceassessment1, Wt as ascsv3, Wp as asxlsx4, Wq as asword1, Wo as aszip3, Wr as aspdf3, JD as exportbutton1 } from './_index-DiaVtc2Z.js';
...
import { U as URL_MODEL_MAP } from './crud-DzBk-fdF.js';
...
import { D as DonutChart } from './DonutChart-DT4dPLNe.js';
import { f as formatDateOrDateTime } from './datetime-CDLVyquZ.js';
import { c as canPerformAction } from './access-control-DaLcieub.js';
import { V as ValidationFlowsSection } from './ValidationFlowsSection-Bt8hUKtU.js';
...
function _page($$payload, $$props) {
  ...
  function accumulateDomainProgress(node) {
    const agg = {
      totalAssessable: 0,
      statusDone: 0,
  ...
  function domainSidebarBarPercent(agg) {
    const t = agg.totalAssessable;
    if (t <= 0) return 0;
    const compliance2 = Math.round(agg.assessedResultCount / t * 100);
    if (compliance2 > 0) return Math.min(100, compliance2);
    if (agg.maxScoreSum > 0) {
      return Math.min(100, Math.round(agg.scoreSum / agg.maxScoreSum * 100));
    }
    return 0;
  }
  function domainWorkflowDonePercent(agg) {
    const t = agg.totalAssessable;
  ...
  let treeCategories = tree ? Object.entries(tree).map(([id2, node], index) => {
    const agg = accumulateDomainProgress(node);
  ...
  let overallDomainsWeightedProgressPct = (() => {
    let wSum = 0;
    let w = 0;
  ...
  let totalAssessableRequirements = assessableNodesCount(treeViewNodes ?? []);
  ...
  function $$render_inner($$payload2) {
    ...
    if (treeViewNodes) {
      $$payload2.out += "<!--[-->";
      $$payload2.out += `${escape_html(totalAssessableRequirements)}`;
    } else {
      $$payload2.out += "<!--[!-->";
    }
    ...
    if (treeCategories.length > 0 && treeViewNodes) {
      ...
      $$payload2.out += `<div class="bg-white rounded-lg border border-gray-200 p-5 shadow-sm"><div class="flex items-center justify-between gap-2 mb-1"><h3 class="text-sm font-semibold text-gray-900">${escape_html(domainscoverage1())}</h3> <span class="bg-gray-100 text-gray-600 px-2.5 py-0.5 rounded-full text-xs font-medium flex-shrink-0"${attr("title", progress())}>${escape_html(overallDomainsWeightedProgressPct)}%</span></div> <p class="text-[11px] text-gray-400 mb-3 leading-snug">${escape_html(safeTranslate("compliance"))}
					· ${escape_html(totalAssessableRequirements)}
					${escape_html(safeTranslate("requirements"))} `;
      if (compliance_assessment.progress_status_enabled) {
        $$payload2.out += "<!--[-->";
        $$payload2.out += `<span class="text-gray-400">· ${escape_html(progress())} (${escape_html(safeTranslate("done"))})</span>`;
      } else {
        $$payload2.out += "<!--[!-->";
      }
      $$payload2.out += `<!--]--></p> <div class="space-y-2"><!--[-->`;
      for (let $$index_6 = 0, $$length = each_array_6.length; $$index_6 < $$length; $$index_6++) {
        let cat = each_array_6[$$index_6];
        const t = cat.agg.totalAssessable;
        const barWidth = cat.progressPct;
        $$payload2.out += `<div><div class="flex items-center justify-between gap-2 text-xs mb-1"><span class="text-gray-600 truncate">${escape_html(cat.index)}. ${escape_html(cat.name)}</span> <span class="text-gray-700 font-medium tabular-nums flex-shrink-0 text-right">${escape_html(cat.agg.assessedResultCount)}/${escape_html(t)} <span class="text-gray-400 font-normal">·</span>${escape_html(cat.progressPct)}% `;
        if (compliance_assessment.progress_status_enabled) {
          $$payload2.out += "<!--[-->";
          $$payload2.out += `<br/> <span class="text-gray-400 font-normal text-[10px]">${escape_html(cat.agg.statusDone)}/${escape_html(t)} ${escape_html(safeTranslate("done"))} · ${escape_html(cat.workflowDonePct)}%</span>`;
        } else {
          $$payload2.out += "<!--[!-->";
        }
        $$payload2.out += `<!--]--></span></div> <div class="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden"><div class="h-full bg-[#0077CC] rounded-full transition-all duration-500"${attr_style(`width: ${stringify(barWidth)}%`)}></div></div></div>`;
      }
      $$payload2.out += `<!--]--></div></div>`;
    } else {
      $$payload2.out += "<!--[!-->";
...
//# sourceMappingURL=_page.svelte-BJutwFWq.js.map
