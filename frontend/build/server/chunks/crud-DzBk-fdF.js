...
import { C as updatethislibrary2, D as loading, E as evidenceindexingsuccess2, F as indexingcompleted1, G as indexinginprogress2, H as evidenceindexingfailed2, I as indexingfailed1, J as indexingnotstarted2, K as libraryoverview1, L as suggestedparentheses1, M as deselect, N as select, O as successfullyupdatedobject2, P as errorupdatingobject2, Q as changestatus1, R as changeimpact1, S as changeeffort1, T as changecsffunction2, U as changepriority1, V as changeattackstage2, W as appliedcontrol1, X as evidence, Y as tasknode1, Z as elementaryaction1 } from './_index-DiaVtc2Z.js';
import { L as LOCALE_DISPLAY_MAP } from './constants-CbUNxZZz.js';
...
function EvidenceFilePreview($$payload, $$props) {
  push();
  let { cell, meta } = $$props;
  function attachmentPathFingerprint(raw) {
    if (!raw) return "";
    try {
  ...
  meta?.id != null ? `/${meta.evidence ? "evidence-revisions" : "evidences"}/${meta.id}/attachment` : null;
  if (meta?.id) {
    $$payload.out += "<!--[-->";
    {
      $$payload.out += "<!--[!-->";
...
function IndexingStatusDisplay($$payload, $$props) {
  push();
  let { cell, meta } = $$props;
  let status = (cell ?? "").toString().toLowerCase();
  let errorMessage = meta?.indexing_error || "";
  if (status === "completed") {
    $$payload.out += "<!--[-->";
    $$payload.out += `<span class="badge preset-tonal-success text-xs"${attr("title", evidenceindexingsuccess2())}><i class="fa-solid fa-check-circle mr-1"></i> ${escape_html(indexingcompleted1())}</span>`;
  } else if (status === "pending" || status === "uploading") {
    $$payload.out += "<!--[1-->";
    $$payload.out += `<span class="badge preset-tonal-warning text-xs"${attr("title", indexinginprogress2())}><i class="fa-solid fa-spinner fa-spin mr-1"></i> ${escape_html(indexinginprogress2())}</span>`;
  } else if (status === "failed") {
    $$payload.out += "<!--[2-->";
    $$payload.out += `<span class="badge preset-tonal-error text-xs"${attr("title", errorMessage || evidenceindexingfailed2())}><i class="fa-solid fa-exclamation-triangle mr-1"></i> ${escape_html(indexingfailed1())}</span>`;
  } else if (status === "not_started") {
    $$payload.out += "<!--[3-->";
    $$payload.out += `<span class="badge preset-tonal-surface text-xs"${attr("title", indexingnotstarted2())}><i class="fa-regular fa-circle mr-1"></i> ${escape_html(indexingnotstarted2())}</span>`;
  } else {
    $$payload.out += "<!--[!-->";
    $$payload.out += `<span class="text-gray-400 text-sm">—</span>`;
  }
...
function AutocompleteSelect($$payload, $$props) {
  ...
  run(() => {
    if (!isInternalUpdate && optionsLoaded && !arraysEqual(selectedValues, store_get($$store_subs ??= {}, "$value", value))) {
      ...
      const formHasValues = multiple ? Array.isArray(store_get($$store_subs ??= {}, "$value", value)) && store_get($$store_subs ??= {}, "$value", value).length > 0 : Boolean(store_get($$store_subs ??= {}, "$value", value));
      if (selectedValues.length === 0 && formHasValues) {
        const valueArray = Array.isArray(store_get($$store_subs ??= {}, "$value", value)) ? store_get($$store_subs ??= {}, "$value", value) : [
...
const listViewFields = {
  ...
  evidences: {
    head: ["name", "file", "folder", "owner", "status", "indexing", "updatedAt", "labels"],
    body: [
      "name",
      "attachment",
      "folder",
...
//# sourceMappingURL=crud-DzBk-fdF.js.map
