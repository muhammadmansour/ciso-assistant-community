"""Login page for the OAuth flow, mounted at GET/POST {PATH}/login."""
...
from .provider import MuhkamAuthorizationCode
...
def _render_form(login_id: str, error: str | None = None) -> str:
    ...
    action = html.escape(config.LOGIN_PATH)
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
<title>Sign in to Muhkam</title>
...
<style>
  ...
  h1 {{ font-size: 20px; margin: 0 0 4px; color: #0f3d3e; }}
  ...
  button {{
    width: 100%; padding: 11px; background: #0f3d3e; color: #fff; border: none;
    border-radius: 6px; font-size: 14px; font-weight: 600; cursor: pointer;
  }}
  button:hover {{ background: #0a2c2d; }}
</style>
</head>
<body>
  <div class="card">
...
async def handle_login(request: Request) -> Response:
    ...
    try:
        pat = login_and_mint_pat(email, password, token_name=f"mukam-mcp-{int(time.time())}")
